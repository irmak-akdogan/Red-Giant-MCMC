...
def fit_stars(data_table: pd.DataFrame, writer):
    writer.writerow("KIC nu_max H P tau alpha W success status".split())

    for (kic_id, row) in data_table.iterrows():
        min_res: OptimizeResult = fit_star(kic_id, row) # type: ignore
        
        out_row = [kic_id] + list(min_res.x) + [min_res.success, min_res.status]
